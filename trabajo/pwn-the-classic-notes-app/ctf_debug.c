...
void create_note(int size)
{
    ...
    printf("[DEBUG] Alloc note %d size %d @ %p\n", note_size, size, note);
    printf("[DEBUG] Alloc notes array size %zu @ %p\n", (size_t)(note_size + 1) * sizeof(char *), new_notes);
    if (notes != NULL)
    {
      ...
      if (note_size >= 2)
      {
          printf("[DEBUG] Dump note %d contents: ", note_size);
      ...
      printf("[DEBUG] Store note index %d pointer %p into array %p\n", note_size, note, new_notes);
      note_size++;
      notes = new_notes;
      printf("[DEBUG] note_size now %d, notes array pointer %p\n", note_size, notes);
}
...
int main(void)
{
  ...
  printf("[DEBUG] my_function_frames @ %p\n", my_function_frames);
    for (int i = 0; i < SIZE_OF_MY_FUNCTION_FRAMES; i++)
    {
