...
cipher_text = Path('cipher_ascii.txt').read_text(encoding='utf-8', errors='ignore')
cipher_indices = [ord(ch) - 65 for ch in cipher_text.upper() if 'A' <= ch <= 'Z']
...
def tetragram_score_from_plain(plain_upper):
    letters = [ord(ch) - 65 for ch in plain_upper if 'A' <= ch <= 'Z']
    total_score = 0.0
    for i in range(len(letters) - 3):
...
def decrypt_with_key(key):
    mapping_upper = {ALPHABET[i]: ALPHABET[key[i]] for i in range(26)}
    out = []
    for ch in cipher_text:
        up = ch.upper()
        if 'A' <= up <= 'Z':
            rep = mapping_upper[up]
            if ch.islower():
...
def combined_score(key):
    ...
    tet_score = tetragram_score_from_plain(plain.upper())
    w_score = word_score(plain)
    total = tet_score + WORD_WEIGHT * w_score
    return total, plain
